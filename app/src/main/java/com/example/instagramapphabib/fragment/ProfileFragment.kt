...
import com.example.instagramapphabib.AccountSettingActivity
import com.example.instagramapphabib.R
import com.example.instagramapphabib.adapter.MyImageAdapter
...
class ProfileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        var recylerViewUploadImage:RecyclerView? = null
        recylerViewUploadImage = viewProfile.findViewById(R.id.recylerview_upload_image)
        recylerViewUploadImage?.setHasFixedSize(true)
        val linearLayoutManager = GridLayoutManager(context,3)
        recylerViewUploadImage?.layoutManager = linearLayoutManager

        postListGrid = ArrayList()
        myImageAdapter = context?.let {
            MyImageAdapter(it,postListGrid as ArrayList<Post>)
        }
        recylerViewUploadImage?.adapter = myImageAdapter
            ...
            when{
                ...
                getButtonText == "Follow" -> {
                        firebaseUser?.uid.let { it1 ->
                            FirebaseDatabase.getInstance().reference
                                ...
                                .child("Followimg").child(profileId).setValue(true)
                        }
                    firebaseUser?.uid.let { it1 ->
                        FirebaseDatabase.getInstance().reference
                ...
                getButtonText == "Following" -> {
                    firebaseUser?.uid.let { it1 ->
                        FirebaseDatabase.getInstance().reference
                            .child("Follow").child(it1.toString())
                            .child("Followimg").child(profileId).removeValue()
                    }
                    firebaseUser?.uid.let { it1 ->
                        FirebaseDatabase.getInstance().reference
                            .child("Follow").child(profileId)
